...
class ODEDuffingSolver(TimeSeriesModel):
    def __init__(
        self, init_vars, init_coeffs, dt, solver="euler", outvar=None, **kwargs
    ):
    ...
    def euler_step(self, prev_val):
        ...
        pred["x"] = prev_val["x"] + prev_val["x_"] * self.dt
        pred["x_"] = prev_val["x_"] + (self.coeffs["g"]*torch.cos(self.coeffs["w"]*prev_val["t"]) - self.coeffs["d"]*prev_val["x_"] - self.coeffs["a"]*prev_val["x"] - self.coeffs["b"]*prev_val["x"]*prev_val["x"]*prev_val["x"]) * self.dt
        pred["t"] = prev_val["t"] + self.dt
        return pred
