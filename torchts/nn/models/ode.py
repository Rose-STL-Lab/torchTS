...
class ODESolver(TimeSeriesModel):
    ...
    def runge_kutta_4_step(self, prev_val):
        ...
        for var in self.var_names:
            pred[var] = (prev_val[var] + (self.ode[var](k_1, self.coeffs) / 6 + self.ode[var](k_2, self.coeffs) / 3 + self.ode[var](k_3, self.coeffs) / 3 + self.ode[var](k_4, self.coeffs) / 6 ) * self.dt
            )
